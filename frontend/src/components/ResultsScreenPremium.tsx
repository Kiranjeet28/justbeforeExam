...
function Button({ onClick, variant, className, children }: ButtonProps) {
  const baseClasses =
    "inline-flex min-h-11 items-center justify-center px-6 py-3 rounded-lg font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 active:scale-[0.98]";
  ...
  return (
    ...
    >
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        >
          <div className="relative w-32 sm:w-40 h-32 sm:h-40 mb-4 sm:mb-6">
            {/* Outer Glow */}
            <div
              className={`absolute inset-0 rounded-full bg-gradient-to-r ${getScoreColor()} opacity-20 blur-xl sm:blur-2xl`}
            />
            ...
            <svg
              ...
              viewBox="0 0 192 192"
              role="img"
              aria-label={`Quiz accuracy ${displayScore}%`}
            >
              <circle
                cx="96"
                cy="96"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span
                className={`text-3xl sm:text-4xl md:text-5xl font-black bg-gradient-to-r ${getScoreColor()} bg-clip-text text-transparent`}
              >
                {displayScore}%
              </span>
        ...
        <motion.div
          ...
          className="grid grid-cols-1 gap-3 sm:gap-4 mb-6 sm:mb-8 sm:grid-cols-2"
        >
          <Card className="p-3 sm:p-4 bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-violet-500/20 backdrop-blur-xl">
            <div className="flex items-center gap-2 mb-2">
              <Trophy size={16} className="text-amber-400 sm:w-5 sm:h-5" />
        ...
        {weakAreas.length > 0 && (
          ...
          >
            <div className="mb-4 flex items-center gap-2">
              <Target size={16} className="text-red-400 sm:w-5 sm:h-5" />
              ...
              {weakAreas.map((area, index) => (
                ...
                >
                  <div className="flex flex-col gap-1 mb-1 sm:flex-row sm:items-center sm:justify-between">
                    <span className="break-words text-xs sm:text-sm font-medium text-red-300">
                      {area.topic}
                    </span>
                    <span className="shrink-0 text-xs text-red-400">
                      {area.percentage}% correct
                    </span>
                  </div>
        ...
        {strongAreas.length > 0 && (
          ...
          >
            <div className="mb-4 flex items-center gap-2">
              <TrendingUp
                size={16}
                className="text-emerald-400 sm:w-5 sm:h-5"
              />
              <h3 className="font-semibold text-sm sm:text-base text-white">
                Your Strengths
              </h3>
            ...
            <div className="space-y-2 mb-6 sm:mb-8">
              {strongAreas.map((area, index) => (
                <motion.div
                  ...
                  className="p-2 sm:p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20"
                >
                  <div className="flex flex-col gap-1 mb-1 sm:flex-row sm:items-center sm:justify-between">
                    <span className="break-words text-xs sm:text-sm font-medium text-emerald-300">
                      {area.topic}
                    </span>
        ...
        >
          <Button
            ...
            className="w-full py-2 sm:py-3 min-h-10 sm:min-h-11 text-sm sm:text-base bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-semibold rounded-xl"
          >
