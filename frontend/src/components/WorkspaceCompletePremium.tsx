...
export function WorkspaceCompletePremium() {
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      const weakAreaTopics = state.quizResults.weakAreas.map(
        (area) => area.topic,
      );
      const recommendations =
        await studyService.getRecommendations(weakAreaTopics);
  ...
  const renderInputStage = () => (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-4 py-8 sm:px-6 md:px-8 lg:p-12 flex items-center justify-center"
    >
      <div className="max-w-2xl w-full">
        ...
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black mb-2 sm:mb-4 bg-gradient-to-r from-violet-400 via-cyan-400 to-violet-400 bg-clip-text text-transparent break-words">
            Just Before Exam
          </h1>
          <p className="text-sm sm:text-base md:text-lg text-slate-300 break-words px-2">
        ...
        <Card className="p-4 sm:p-6 md:p-8 bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-violet-500/20 backdrop-blur-xl">
          <div className="space-y-4 sm:space-y-6">
            <div>
              ...
              {inputUrls.map((url, index) => (
                <motion.div
                  ...
                  className="mb-2 sm:mb-3 flex flex-col gap-2 sm:flex-row sm:gap-3"
                >
                  <input
                    ...
                    className="min-h-11 sm:min-h-12 min-w-0 flex-1 px-3 sm:px-4 py-2 sm:py-3 rounded-lg bg-slate-900/50 border border-violet-500/20 text-white placeholder-slate-400 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 focus:border-violet-500/50 focus:bg-slate-900/70 transition-all"
                  />
                  {inputUrls.length > 1 && (
                    <Button
                      onClick={() =>
                      ...
                      variant="secondary"
                      className="shrink-0 px-3 py-2 sm:py-3 min-h-11 sm:min-h-12"
                    >
                      Remove
                    </Button>
                  )}
            ...
            <Button
              ...
              variant="secondary"
              className="w-full py-2 sm:py-3 min-h-11 sm:min-h-12"
            >
              <PlusCircle size={16} className="mr-2 sm:mr-3" /> Add Another
              Material
            </Button>
            ...
            <Button
              ...
              className="w-full py-2 sm:py-3 min-h-11 sm:min-h-12 text-sm sm:text-base bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-semibold rounded-xl"
            >
      ...
      {state.topics.length > 0 && (
        ...
        >
          ...
          <motion.div
            ...
            className="mt-6 sm:mt-8 flex flex-col gap-2 sm:gap-3 sm:flex-row"
          >
            <Button
              onClick={() => handleGenerateQuiz()}
              className="flex-1 py-2 sm:py-3 min-h-11 sm:min-h-12 text-sm sm:text-base bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-semibold rounded-xl"
            >
              Start Quiz
            </Button>
            <Button
              onClick={handleReset}
              variant="secondary"
              className="flex-1 sm:flex-none py-2 sm:py-3 min-h-11 sm:min-h-12 sm:px-6"
            >
              Reset
            </Button>
          </motion.div>
  ...
  const renderRecommendations = () => (
    ...
    >
      <div className="max-w-4xl mx-auto">
        ...
        >
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-violet-400 via-cyan-400 to-violet-400 bg-clip-text text-transparent mb-2 break-words">
            Recommended Resources
          </h1>
          <p className="text-xs sm:text-sm text-slate-400 break-words">
            Based on your quiz performance, here are resources to strengthen
            your weak areas
          </p>
        </motion.div>
        ...
        {state.isLoading ? (
          ...
          >
            {[0, 1, 2].map((item) => (
              <div
                key={item}
                className="h-24 sm:h-28 rounded-lg border border-violet-500/10 bg-slate-800/30 animate-pulse"
              />
            ))}
          </div>
        ) : (
          <div className="grid gap-3 sm:gap-4">
            {state.recommendations.map((rec, index) => (
              <motion.div
                ...
                transition={{ delay: index * 0.1 }}
              >
                <Card className="p-4 sm:p-6 bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-violet-500/20 backdrop-blur-xl hover:border-violet-500/40 transition-all">
                  <div className="flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-start">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs sm:text-sm text-slate-400 mb-2 sm:mb-3 break-words">
                        {rec.description}
                      </p>
                      <div className="flex gap-2 flex-wrap">
                        <span className="text-xs px-2 sm:px-3 py-1 rounded-full bg-violet-500/20 text-violet-300">
                          {rec.type}
                        </span>
                        <span className="text-xs px-2 sm:px-3 py-1 rounded-full bg-cyan-500/20 text-cyan-300">
                          {rec.topic}
                        </span>
                      </div>
                    </div>
        ...
        >
          <Button
            ...
            variant="secondary"
            className="flex-1 py-2 sm:py-3 min-h-11 sm:min-h-12 text-sm sm:text-base"
          >
            Retake Quiz
          </Button>
          <Button
  ...
  const renderError = () => (
    <motion.div
      ...
      className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-4 py-8 sm:px-6 md:p-12 flex items-center justify-center"
    >
      <Card className="max-w-md w-full p-4 sm:p-6 border border-red-500/20 bg-red-500/5">
        <div className="flex items-start gap-2 sm:gap-3">
          <AlertCircle className="text-red-400 flex-shrink-0 mt-1" size={20} />
          <div className="flex-1">
            <h3 className="font-semibold text-red-300 mb-1 text-sm sm:text-base">
              Error
            </h3>
            <p className="text-red-200 text-xs sm:text-sm mb-3 sm:mb-4 break-words">
              {state.error}
            </p>
            <Button
              onClick={handleReset}
              variant="secondary"
              className="py-2 sm:py-3 min-h-10 sm:min-h-11 text-xs sm:text-sm text-red-300 border-red-500/30 hover:border-red-500"
            >
              Try Again
            </Button>
          </div>
